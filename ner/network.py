...
class NER:
    def __init__(self,
        ...
        if 'cnn' in net_type.lower():
            ...
            with tf.variable_scope('HighwayConvNet'):
        ...
        if use_capitalization:
            units = tf.concat([units, cap], axis=2)

        # Classifier
        with tf.variable_scope('Classifier'):
            if dense_dropout:
                units = tf.layers.dropout(units, dropout_ph)
            if two_dense_layers:
                units = tf.layers.dense(units, n_filters[-1])
        ...
        loss = tf.reduce_mean(loss_tensor) + l2_beta * l2_loss
    ...
    def load(self, model_file_path):
        vars = [var for var in tf.trainable_variables() if 'Adam' not in var.name]
    ...
    def fit(self,
        ...
        for epoch in range(epochs):
            ...
            for x, y in batch_generator:
                ...
                self._sess.run(self._train_op, feed_dict=feed_dict)
            result = self.eval_conll('valid', print_results=False)
            current_f1 = result['__total__']['f1']
            if current_f1 > current_best:
                print('Current best f1 score updated: {}'.format(current_f1))
                current_best = current_f1
                self.save(save_path)
                current_patience = patience
            else:
                current_patience -= 1
    ...
    def get_train_op(self, loss, learning_rate, learnable_scopes=None, lr_decay_rate=None):
        ...
        with tf.control_dependencies(extra_update_ops):
            ...
            # Gradient clipping to eliminate exploding gradients.
            clip_norm = 1.0
